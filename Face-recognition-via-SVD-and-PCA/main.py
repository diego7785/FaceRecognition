...
if __name__ == '__main__':
    ...
    #path='/home/diego/Escritorio/Truora/FaceRecognition-20191015T010452Z-001/FaceRecognition/VideoDiego/video.mp4'
    path='/home/diego/Escritorio/Truora/FaceRecognition-20191015T010452Z-001/FaceRecognition/Rostros/George.png'
    if video_cam_recognition:
        ...
        detected_face_gray, detectec_face_coords = detect_face(img_gray)
        img_with_mask = None
        if detected_face_gray is not None:
            mask = np.zeros_like(image)
            draw_rectangles(mask,[detectec_face_coords])
            frame_with_mask = cv2.add(image,mask)
        img_to_show = img_with_mask if img_with_mask is not None else image
        cv2.imshow('Imagen',img_to_show)
        k=cv2.waitKey(0)

        if k % 256 == 32 and detected_face_gray is not None:
            print('Recognizing the images')
